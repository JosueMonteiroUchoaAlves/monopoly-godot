class_name MuseumData extends PropertyData

@export var money_per_art: int = 20

func _init():
	type = ItemType.arts
	context_type = MuseumContext
	
func charge_rent(event: RentContext, turn: TurnContext, context: PropertyContext):
	super(event, turn, context)
	if is_instance_of(context._owner, MainController):
		var art_count = 0
		for key in context._owner.inventory:
			for item in context._owner.inventory[key]:
				if item.type == ItemType.arts:
					art_count += 1
		var extra_value = art_count * money_per_art
		event.payer.gain_or_spend_money(extra_value * -1)
		context._owner.gain_or_spend_money(extra_value)
